...
CREATE OR REPLACE procedure DNS_id_query (  
...
BEGIN
 /* 
    Replacement sproc for the PowerDNS gmysql-id-query for AllStarLink
...
/* now the replacement for # gmysql-id-query */
...
/* Now lets process the records from the srvCursor query */
       ...
       IF remoteBase = '1' THEN
        INSERT INTO tempRecords(content,proc_ttl,prio,type,temp_domain_id,disabled,temp_name,proc_auth) 
          SELECT (CONCAT(srvWeight, ' ', udp, ' ', nodeNumber,'.RemoteBase.', specialSubDomain)), proc_ttl, proc_prio,
            IN_type, proc_domain_id, disabled, IN_name, proc_auth;
      ...
      CLOSE srvCursor;
  END IF;
  /* now do a lookup for the SRV record in the normal table */
 INSERT INTO tempRecords(content,proc_ttl,prio,type,temp_domain_id,disabled,temp_name,proc_auth) 
  SELECT content,ttl,prio,type,domain_id,disabled,name,auth FROM records WHERE disabled=0 AND
    type = 'SRV' AND name = IN_name AND domain_id = proc_domain_id;


/* Now see if it's an A record query */
ELSEIF IN_type = 'A' THEN
/* see if it matches the remotebase or nodes subdomain */
 IF SUBSTRING_INDEX(IN_name,'.',-4) = (CONCAT('remotebase.', specialSubDomain))  THEN
      SET nodeNumber = SUBSTRING_INDEX(IN_name,(CONCAT('.remotebase.', specialSubDomain)), 1);
 ELSEIF IN_type = 'A' and SUBSTRING_INDEX(IN_name,'.',-3) = specialSubDomain THEN
...
/* ok not if proxy IP is null, then return the ipaddr */
       IF aProxy_IP IS null THEN
        INSERT INTO tempRecords(content,proc_ttl,prio,type,temp_domain_id,disabled,temp_name,proc_auth) 
          SELECT aIPaddr, proc_ttl, proc_prio, IN_type, proc_domain_id, disabled, IN_name, proc_auth;
  ...
  /* now we need to select the A records matching from the records table and add them */
 INSERT INTO tempRecords(content,proc_ttl,prio,type,temp_domain_id,disabled,temp_name,proc_auth) 
  SELECT content,ttl,prio,type,domain_id,disabled,name,auth FROM records WHERE disabled=0 AND
    type = 'A' AND name = IN_name AND domain_id = proc_domain_id;

/* Now see if it's an TXT record query */
ELSEIF IN_type = 'TXT' THEN
/* see if it matches the remotebase or nodes subdomain */
 IF SUBSTRING_INDEX(IN_name,'.',-4) = (CONCAT('remotebase.', specialSubDomain))  THEN
      SET nodeNumber = SUBSTRING_INDEX(IN_name,(CONCAT('.remotebase.', specialSubDomain)), 1);
...
/* put each record in the temp table */
  ...
  /* now we need to select the TXT records matching from the records table and add them */
 INSERT INTO tempRecords(content,proc_ttl,prio,type,temp_domain_id,disabled,temp_name,proc_auth) 
  SELECT content,ttl,prio,type,domain_id,disabled,name,auth FROM records WHERE disabled=0 AND
    type = 'TXT' AND name = IN_name AND domain_id = proc_domain_id ;

END IF;



/* ok, type is not SRV, A or TXT, so just do a normal lookup */
IF ((IN_type <> 'TXT') AND (IN_type <> 'SRV' ) AND (IN_type <> 'A' )) THEN 
  INSERT INTO tempRecords(content,proc_ttl,prio,type,temp_domain_id,disabled,temp_name,proc_auth) 
    SELECT content,ttl,prio,type,domain_id,disabled,name,auth FROM records WHERE disabled=0 AND
      type = IN_type AND name = IN_name AND domain_id = proc_domain_id ;
END IF;
